...
-   **Chess** – Nearly complete; core mechanics implemented, pending final king safety logic.
-   **Circle Head** – Fully playable arena survival shooter; features boss fight at wave 5 and endless waves for high-score challenges.
...
The current repository includes all games that are **available on this device**.  
Some games, or **better/updated versions** of existing games, are not included because the original files are inaccessible (e.g., created on older devices or devices used for competitions).

Examples of such missing projects include:

-   **Dino** – Chrome offline game-inspired; similar mechanics, but player controls a running character instead of a dino.
-   **Flappy Bird** – Side-scrolling bird game clone; navigate the bird through pipes.
-   **Duet** – Play Store-inspired twin-rotation game; survive as long as possible by controlling two orbiting objects.
-   Other experimental or competition-specific projects

These are mentioned for reference and completeness; source code or playable versions are not included in this repository.

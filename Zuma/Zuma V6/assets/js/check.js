...
function checkGameOver(id) {
    if (balls[id].length) {
        ...
        if (headBall.percent >= 1 || gameOver === id) {
            if (!gameOver) {
                sound('light-trail');
            }

            gameOver = id; //default (true)
            ...
            if (!balls[id].length) {
                ...
                render = false;
                gameOverContainer.querySelector('h1').innerHTML = `Player ${!parseInt(id) * 1 + 1} [${players[!parseInt(id) * 1].name}] WIN`;

class Ball {
    constructor(color = '') {
        ...
        this.push.speed = convert(this.push.speed);
    ...
    update() {
        if (!gameOver) {
            if ((this.push.target > 0 && this.push.direction === 1) || (this.push.target < 0 && this.push.direction === -1)) {
                let check = this.push.direction === 1 ? this.push.target - this.push.speed <= 0 : this.push.target - this.push.speed >= 0;
                let pushSpeed = this.push.direction * (check ? this.push.target : this.push.speed);

                this.percent += pushSpeed;
                this.push.target -= pushSpeed;

                if (!this.push.target && this.push.direction === 1) {
                    let frontBallIndex = balls.indexOf(this) - 1;
                    let frontBall = balls[frontBallIndex];
                    ...
                    if (frontBall) {
                        ...
                        if (percentDifference < ball.margin) {
                            ...
                            for (let i = frontBallIndex; i >= 0; i--) {
                                balls[i].push.target = pushMargin;
                            }
                        }
                    }
                }
        ...
        if (startingPhase) {
            ...
            speed *= scoreMultiplier;
        }

        this.percent += this.direction * speed;
    ...
    updateRotationDifference() {
        ...
        if (!this.rotate.currentDifference) {
            // render = false;
            checkSameBalls(balls.indexOf(this));
        }
    }
